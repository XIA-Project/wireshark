...
#define MIN_XDGRAM_HEADER_SIZE 4
...
dissect_xdgram(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    ...
    if (actual_len < MIN_XDGRAM_HEADER_SIZE) {
        return 0;
    }
    ...
    hlen = tvb_get_guint8(tvb, XDGRAM_HLEN) << 2;

    ti = proto_tree_add_protocol_format(tree, proto_xdgram, tvb, 0, hlen, "XIA Xdatagram Protocol");
    xdgram_tree = proto_item_add_subtree(ti, ett_xdgram);

    proto_tree_add_item(xdgram_tree, hf_xdgram_next_hdr, tvb, XDGRAM_NXTH, 1, ENC_BIG_ENDIAN);
    proto_tree_add_uint(xdgram_tree, hf_xdgram_hlen, tvb, XDGRAM_HLEN, 1, hlen);
    proto_tree_add_item(xdgram_tree, hf_xdgram_extra, tvb, XDGRAM_EXTRA, 2, ENC_BIG_ENDIAN);
...
proto_register_xdgram(void)
{
    static hf_register_info hf[] = {
		{&hf_xdgram_next_hdr, {"Next Header", "xdgram.next_hdr", FT_UINT8,
			BASE_HEX, next_header_vals, 0x0, NULL, HFILL}},

		{&hf_xdgram_hlen, {"Header Length", "xdgram.hlen", FT_UINT8,
		BASE_DEC, NULL, 0x0, NULL, HFILL}},

		{&hf_xdgram_extra, {"Extra Bytes", "xdgram.extra", FT_UINT16,
    ...
    xdgram_handle = register_dissector("xdatagram", dissect_xdgram, proto_xdgram);
}
